    ...
    try {
      ...
      try {
        ...
        allDocsSnapshot.docs.forEach((doc, idx) => {
          const data = doc.data();
          const rutConductorDoc = (data.rutConductor || '').toString().trim();
          const rutConductorDocNormalizado = normalizarRut(rutConductorDoc);
          const tipoUsuarioDoc = (data.tipoUsuario || '').toString().trim();
          
          // Verificar si coincide con nuestro RUT (normalizado o original)
          const coincideRUT = rutConductorDocNormalizado === rutNormalizado || 
                            rutConductorDoc === rutGuardado ||
                            rutConductorDocNormalizado === normalizarRut(rutGuardado);
          
          if (coincideRUT && tipoUsuarioDoc === 'conductor') {
          ...
          if (idx < 10) {
            ...
            console.log(`     - rutConductor: "${rutConductorDoc}" (normalizado: "${rutConductorDocNormalizado}")`);
            console.log(`     - rutApoderado: "${data.rutApoderado || 'N/A'}"`);
            console.log(`     - tipoUsuario: "${tipoUsuarioDoc}"`);
            console.log(`     - fechaViajeFormateada: "${data.fechaViajeFormateada || 'N/A'}"`);
            console.log(`     - Coincide con nuestro RUT: ${coincideRUT && tipoUsuarioDoc === 'conductor' ? '✅ SÍ' : '❌ NO'}`);
          }
        });
        
        console.log(`🔍 Documentos que coinciden con nuestro RUT y tipoUsuario='conductor': ${documentosCoincidentes.length}`);
      ...
      if (!consultaExitosa) {
        ...
        try {
          // Consultar TODOS los documentos de tipo 'conductor' y filtrar por RUT en memoria
          ...
          console.log(`📋 Documentos encontrados (tipoUsuario='conductor'): ${snapshotGeneral.docs.length}`);
      ...
      if (viajesLista.length > 0) {
        ...
        console.warn('  2. ¿El RUT del conductor coincide?');
